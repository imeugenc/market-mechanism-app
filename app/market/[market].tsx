...
import { canAccessPremiumContent, groupAnalysesByDate } from "@/features/content/access";
...
export default function MarketDetailScreen() {
  ...
  const { analyses, membership, reviews, saveMarket, trackView } = useAppState();
    ...
    <Screen>
      ...
      <PremiumCard>
        ...
        <View style={styles.actions}>
          ...
          />
          <PrimaryButton label="Salvează piața" variant="ghost" onPress={() => market && saveMarket(market)} />
        </View>
      </PremiumCard>
      ...
      {marketReviews.length ? (
        marketReviews.map((review) => <ReviewCard key={review.id} item={review} />)
      ) : (
            ...
            {group.analyses.map((item) => {
              ...
              return (
                <View key={item.id} style={styles.videoCard}>
                  <DailyVideoCard item={item} locked={itemLocked} />
      ...
      <View style={styles.ctaCard}>
        ...
        <PrimaryButton label="Deschide Analize personale" onPress={() => router.push("/requests")} />
      </View>
    </Screen>
  );
}
