...
export function MarketDetailScreen() {
  ...
  const { analyses, dailyBiases, favorites, membership, publicContentStates, reviews, toggleFavorite, trackView } = useAppState();
  ...
  const marketStates = [publicContentStates.analyses, publicContentStates.biases, publicContentStates.reviews];
  const contentLoading = marketStates.includes("loading");
  const contentError = marketStates.every((state) => state === "error");
  const contentPartial = !contentLoading && !contentError && marketStates.includes("error");
  const showSections = hasAnyContent || contentLoading || contentPartial;
  ...
  return (
    <Screen>
      ...
      {contentError ? <ContentStatePanel kind="error" /> : null}
      {contentPartial ? <ContentStatePanel kind="error" title="O parte din conținut nu s-a încărcat" message="Datele disponibile pentru această piață sunt afișate mai jos." compact /> : null}
      {!contentLoading && !contentError && !contentPartial && !hasAnyContent ? <ContentStatePanel kind="empty" title={`Nu există încă conținut publicat pentru ${normalizedMarket}`} message="Piața rămâne disponibilă. Conținutul va apărea după prima publicare." /> : null}

      {showSections && view === "current" ? (
        <>
          {publicContentStates.analyses === "ready" && publicContentStates.biases === "ready" && !todayAnalysis && !todayBias ? <ContentStatePanel kind="empty" title="Niciun briefing sau Bias publicat astăzi" message={latestAnalysis || latestBias ? "Cel mai recent conținut rămâne disponibil în Istoric." : "Revino după următoarea publicare."} compact /> : null}

          <SectionHeader eyebrow="Daily Bias" title={todayBias ? "Bias-ul curent" : "Cel mai recent Bias"} />
          {publicContentStates.biases === "loading" ? <ContentStatePanel kind="loading" title="Se încarcă Daily Bias…" compact /> : publicContentStates.biases === "error" ? <ContentStatePanel kind="error" title="Daily Bias nu s-a încărcat" compact /> : (todayBias ?? latestBias) ? (
            <Pressable style={styles.biasCard} onPress={() => router.push(`/bias/${(todayBias ?? latestBias)!.id}` as Href)}>
              ...
              <Text style={styles.biasConfidence}>Încredere: {({ Low: "scăzută", Medium: "medie", High: "ridicată" } as const)[(todayBias ?? latestBias)!.confidence]}</Text>
          ...
          {publicContentStates.analyses === "loading" ? <ContentStatePanel kind="loading" title="Se încarcă briefingul…" compact /> : publicContentStates.analyses === "error" ? <ContentStatePanel kind="error" title="Briefingul nu s-a încărcat" compact /> : (todayAnalysis ?? latestAnalysis) ? (
